...
class ImageToPatches(nn.Module):
    ...
    def forward(self, x):
        assert len(x.size()) == 4
        y = self.unfold(x)      # Size B, C*patch_size*patch_size, num_paches
        y = y.permute(0, 2, 1)  # Size B, num_paches, C*patch_size*patch_size
        return y
    # end def
# end class


# The PatchEmbedding layer takes multiple image patches in (B,T,Cin) format (T patches per image)
